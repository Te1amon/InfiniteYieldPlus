...
Title.TextSize = 18
Title.Text = "Infinite Yield FE (Watchdog)"
...
CMDs[#CMDs + 1] = {NAME = 'truesight', DESC = 'Allows you to see fake watchdog entities'}
...
addcmd('fly',{},function(args, speaker)
	...
	if pchar and not pchar:FindFirstChild(floatName) then
		task.spawn(function()
			...
			notify('Fly','Watchdog Fly Enabled (Q = down & E = up)')
...
addcmd('unfly',{'nofly','novfly','unvehiclefly','novehiclefly','unvfly'},function(args, speaker)
		Floating = false
	local pchar = speaker.Character
	notify('Fly','Watchdog Fly Disabled')
...
notify('VFly','VFly cannot be enabled.')
end)
...
addcmd('jpower',{'jumppower','jp'},function(args, speaker)
	...
	if isNumber(jpower) then
	   for i,v in pairs(workspace.WATCHDOG_BOT:GetChildren()) do
	       if v:IsA("Part") then
v.CanCollide = false
v.CanTouch = false
else
...
notify("Reach", "Reach cannot be enabled.")
end)
...
for i,v in pairs(workspace.Watchdog2:GetChildren()) do
    if v:IsA("Part") then
        v.Transparency = 0.5
        else
...
for i,v in pairs(workspace.Watchdog3:GetChildren()) do
    if v:IsA("Part") then
        v.Transparency = 0.5
        else
...
for i,v in pairs(workspace.Watchdog4:GetChildren()) do
    if v:IsA("Part") then
        v.Transparency = 0.5
        else
...
for i,v in pairs(workspace.Watchdog5:GetChildren()) do
    if v:IsA("Part") then
        v.Transparency = 0.5
        else
